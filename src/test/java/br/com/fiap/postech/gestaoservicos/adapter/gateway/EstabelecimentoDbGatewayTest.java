...
@SpringBootTest
public class EstabelecimentoDbGatewayTest {
    ...
    void deveCriarEstabelecimento() {
        ...
        EstabelecimentoDbEntity estabelecimentoDb = this.estabelecimentoMapper.toEstabelecimentoDbEntity(estabelecimento);
        estabelecimentoDb.setId(UUID.randomUUID());

        when(estabelecimentoDbRepository.save(any(EstabelecimentoDbEntity.class)))
                .thenReturn(estabelecimentoDb);

        EstabelecimentoEntity estabelecimentoSalvo =
                this.estabelecimentoRepository.criarEstabelecimento(estabelecimento);

        verify(this.estabelecimentoDbRepository, times(1)).save(any(EstabelecimentoDbEntity.class));
        ...
        assertThat(IntStream.range(0,estabelecimentoSalvo.getProfissional().size()).boxed().collect(
                ...
                .allSatisfy((i, idx)->{
                    ...
                    assertThat(IntStream.range(0, agenda.getAgendamento().size()).boxed().collect(
                            toMap(
                                    idxl -> idxl,
                                    idxl -> agenda.getAgendamento().get(idxl
                                    ))))
                            .allSatisfy((l, idxl)-> {
                                assertThat(agenda.getAgendamento().get(l))
                                ...
                                assertThat(agenda.getAgendamento().get(l).getServico())
    ...
    void deveBuscarPorId() {
        ...
        when(estabelecimentoDbRepository.findById(any(UUID.class)))
                .thenReturn(Optional.of(estabelecimentoDb));

        EstabelecimentoEntity estabelecimentoSalvo =
                this.estabelecimentoRepository.buscarPorId(id);
